...
INPUT_DIM = 128         # Обрезанный спектр
BATCH_SIZE = 4096       
EPOCHS = 20             
LEARNING_RATE = 1e-4    
...
def main():
    ...
    # --- СОЗДАНИЕ МОДЕЛИ ---
    # Мы берем класс из vq_vae.py, где уже прописан EMA
    model = EEG_VQ_VAE(
        input_dim=INPUT_DIM, 
        hidden_dim=256,       # Используем параметры пошире
        embedding_dim=EMBEDDING_DIM, 
        num_embeddings=NUM_EMBEDDINGS
    ).to(DEVICE)
    
    # Инициализация словаря (обязательно!)
    model.init_codebook_from_data(train_loader, DEVICE)
    
    optimizer = torch.optim.Adam(model.parameters(), lr=LEARNING_RATE)
    
    print("Начинаем обучение (EMA mode)...")
    ...
    # Сохранение (имя файла то же, перезапишем старый)
    torch.save(model.state_dict(), "vq_vae_final.weights")
    ...
    plt.tight_layout()
    plt.savefig("final_results_ema.png")
    print("Готово!")
